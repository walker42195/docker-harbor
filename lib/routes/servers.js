...
function createServersRouter({ mw, hub, unlock, agentImage, hubWsUrlOverride }) {
  ...
  // Vad en rensning skulle ta bort. Ren lasning, ingen upplasning kravs.
  router.get('/:serverId/system/prune-preview', requireAuth, resolveServer, requireOnline, async (req, res) => {
    try {
      const data = await req.transport.call('system.pruneInfo', {});
      res.json({ success: true, preview: data });
    } catch (err) {
      sendError(res, err, 'Kunde inte hämta rensningsförhandsvisning: ');
    }
  });

  router.post('/:serverId/system/prune', requireAuth, resolveServer, requireOnline, requireUnlocked, async (req, res) => {
    try {
      const data = await req.transport.call('system.prune', req.body || {});
...
// Slår upp en server via en giltig, ej utgången enrollment-kod.
function findByEnrollCode(hub, code) {
  const now = Date.now();
  return hub.registry.list().find(s =>
    s.type === 'agent' && s.enrollHash && s.enrollExpires > now &&
    require('../registry').compareScrypt(code, s.enrollSalt, s.enrollHash)
  ) || null;
}

// Strömmar ut agent-imagen som en tar, så fjärrservern kan hämta den direkt
// från hubben utan att behöva ett register eller SSH. Skyddas av samma
// engångskod som installationsskriptet.
function createInstallImageRoute({ hub, agentImage }) {
  return async (req, res) => {
    const entry = findByEnrollCode(hub, req.params.code);
    if (!entry) {
      return res.status(404).type('text/plain').send('Ogiltig eller utgangen installationskod.\n');
    }

    const local = hub.getTransport('local');
    if (!local || !local.docker) {
      return res.status(503).type('text/plain').send('Hubben har ingen lokal Docker-anslutning.\n');
    }

    try {
      const stream = await local.docker.getImage(agentImage).get();
      res.setHeader('Content-Type', 'application/x-tar');
      res.setHeader('Content-Disposition', 'attachment; filename="harbor-agent-image.tar"');
      console.log(`[install] serverar agent-imagen "${agentImage}" till "${entry.id}"`);
      stream.pipe(res);
      stream.on('error', () => res.destroy());
    } catch (err) {
      console.error(`[install] kunde inte exportera "${agentImage}":`, err.message);
      res.status(404).type('text/plain').send(
        `Agent-imagen "${agentImage}" finns inte på hubben.\n` +
        'Bygg den där med:  docker build -f agent/Dockerfile -t ' + agentImage + ' .\n'
      );
    }
  };
}

// Image-id:t, sa installationsskriptet kan avgora om den lokala imagen ar
// inaktuell. Utan det hoppade en ominstallation over nedladdningen sa fort
// nagon image med ratt tagg fanns, och agenten kunde aldrig uppdateras.
function createInstallImageIdRoute({ hub, agentImage }) {
  return async (req, res) => {
    const entry = findByEnrollCode(hub, req.params.code);
    if (!entry) return res.status(404).type('text/plain').send('');
    const local = hub.getTransport('local');
    if (!local || !local.docker) return res.status(503).type('text/plain').send('');
    try {
      const info = await local.docker.getImage(agentImage).inspect();
      res.type('text/plain').send(String(info.Id));
    } catch (err) {
      res.status(404).type('text/plain').send('');
    }
  };
...
function createInstallRoute({ hub, agentImage, hubWsUrlOverride }) {
  return (req, res) => {
    ...
    const entry = findByEnrollCode(hub, code);
...
module.exports = { createServersRouter, createInstallRoute, createInstallImageRoute, createInstallImageIdRoute };
