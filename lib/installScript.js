...
INSTALL_DIR="\${HARBOR_INSTALL_DIR:-/opt/docker-harbor-agent}"
MARKER="\$INSTALL_DIR/.docker-harbor-agent"

# Installera aldrig over nagon annans installation. Flera Harbor-projekt
# anvander namnet "harbor-agent", och att skriva i deras katalog -- eller att
# fa nagon att rensa den -- slar sonder deras agent.
if [ -d "\$INSTALL_DIR" ] && [ ! -f "\$MARKER" ] && [ -n "$(ls -A "\$INSTALL_DIR" 2>/dev/null)" ]; then
  echo "FEL: \$INSTALL_DIR finns redan och verkar tillhora nagot annat." >&2
  echo "     Innehall:" >&2
  ls -A "\$INSTALL_DIR" 2>/dev/null | sed "s/^/       /" >&2
  echo "     Radera den INTE utan att veta vad det ar." >&2
  echo "     Valj en annan katalog:" >&2
  echo "       curl -fsSL ${hubHttpUrl}/install/${enrollCode} | sudo HARBOR_INSTALL_DIR=/opt/min-agent sh" >&2
  exit 1
fi
...
detect_project_roots() {
  ...
  docker inspect $ids \
    ...
    | while read -r f; do
        [ -n "$f" ] || continue
        # Hoppa over var egen installation -- annars harleds hela katalogen
        # ovanfor den (t.ex. /opt) som en projektrot och monteras in.
        case "$f" in "$INSTALL_DIR"/*) continue ;; esac
        d=$(dirname "$f")
        case "$d" in
...
if [ "\${HARBOR_ALLOW_FILE_READ:-true}" = "false" ]; then
  ...
  for d in $(detect_project_roots); do
    case "$d" in
      /|/home|/root|/usr|/etc|/var|/opt) continue ;;
    esac
    [ -d "$d" ] || continue
...
LOCAL_IMAGE_ID=$(docker image inspect ${agentImage} --format '{{.Id}}' 2>/dev/null || true)
HUB_IMAGE_ID=$(curl -fsSL "${hubHttpUrl}/install/${enrollCode}/image-id" 2>/dev/null || true)

if [ -n "$LOCAL_IMAGE_ID" ] && [ -n "$HUB_IMAGE_ID" ] && [ "$LOCAL_IMAGE_ID" = "$HUB_IMAGE_ID" ]; then
  echo "==> Agent-imagen ar redan den senaste"
elif [ -n "$LOCAL_IMAGE_ID" ] && [ -z "$HUB_IMAGE_ID" ]; then
  echo "==> Kunde inte fraga hubben om imageversion, behaller den lokala"
else
  if [ -n "$LOCAL_IMAGE_ID" ]; then
    echo "==> Agent-imagen ar inaktuell, hamtar ny fran hubben..."
  else
...
echo "Folj loggen med:  cd \$INSTALL_DIR && \$COMPOSE logs -f docker-harbor-agent"
echo ""
